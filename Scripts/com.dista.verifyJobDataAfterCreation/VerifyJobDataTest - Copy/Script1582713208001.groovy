...
for(int j=0; j<values.size();j++){
	...
	for(int i=0;i<headersCount.SIZE;i++){
			...
			if(isPresent){
				...
				WebUI.scrollToElement(TestDataObject, 0)
				try{
					if(WebUI.verifyEqual(testData, rowValues.get(i))){
						println("Test data validated - "+ testData) 
					}
					else{
						println("Test data not found - "+ testData)
					}
				}
				catch (Exception e){
						println("Retry")
				...
				dropDownTestData = driver.findElement(By.xpath('//div[@name="'+headers.get(i)+'"]')).getAttribute("value")
																		
				try{
					if(WebUI.verifyEqual(dropDownTestData, rowValues.get(i))){
						println("Test data validated - "+ dropDownTestData) 
					}
					else{
						println("Test data not found - "+ testData)
					}
